...
export class RuxPopUpMenu extends LitElement {
    ...
    connectedCallback() {
        ...
        this._trigger = this.parentElement.querySelector(
    ...
    handleOutsideClick(e) {
        const target = e
            ...
            .find(
                (element) =>
                    element.id &&
                    element.id === this._trigger.getAttribute('aria-controls')
            )
        target
    ...
    handleMenuItemClick(e) {
        this.selected = this.data.find(
            (item) => item.id === e.currentTarget.dataset.key
        )
    ...
    _setMenuPosition() {
        ...
        if (
            menuBounds.height + triggerBounds.bottom + padding >
            window.innerHeight
        ) {
