...
export class RuxTree extends LitElement {
   ...
   */
  toggle() {
    this._activeElement.getAttribute('aria-expanded') === 'true' ? this.collapse() : this.expand();
  }
  ...
  next() {
    this._activeElement =
      this._activeElement.getAttribute('aria-expanded') === 'true'
        ...
        : this.findNextSibling(this._activeElement.closest('[role="treeitem"]'));
  }
  ...
  back() {
    this._activeElement = !this._activeElement.previousElementSibling
      ...
      : this._activeElement.previousElementSibling.getAttribute('aria-expanded') === 'false'
      ? this._activeElement.previousElementSibling
      : this.findPreviousSibling(this._activeElement.previousElementSibling.closest('[role="treeitem"]')) ||
        this._activeElement;
  }
  ...
  findPreviousSibling(element) {
    const lastChild = element.querySelector('[role="group"]').lastElementChild;

    return lastChild.getAttribute('aria-expanded') === 'false' ? lastChild : this.findPreviousSibling(lastChild);
  }
   ...
   */
  select() {
    this.selected = this.findInObject(this.data, this._activeElement.dataset.key);
  ...
  updated(changedProperties) {
    if (changedProperties && changedProperties.get('_activeElement')) {
      const _previousActiveElement = changedProperties.get('_activeElement');
